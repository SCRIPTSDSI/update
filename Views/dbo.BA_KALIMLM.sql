...
AS
      ...
      SELECT --TOP 100 PERCENT 
             ...
             B.LLOGARI,
             LLOGARIPK=B.LLOGARI
        FROM BANKASCR A LEFT JOIN KLIENT   B ON A.LLOGARIPK = B.KOD, CONFIGLM C
       WHERE A.TIPKLL='S'
   ...
   UNION ALL
      ...
      SELECT --TOP 100 PERCENT 
             ...
             B.LLOGARI,
             B.LLOGARI
        FROM BANKASCR A LEFT JOIN FURNITOR B ON A.LLOGARIPK = B.KOD, CONFIGLM C
       WHERE A.TIPKLL='F'
        ...
        FROM
          (
             SELECT --TOP 100 PERCENT 
                    ...
                    A.RRAB,
                    DEP       = CASE WHEN ISNULL(R3.KALIMBALMDEPLIST,0)=0          -- RRAB='K' Or 
                                     THEN ''
                                     ELSE ISNULL(dbo.Isd_SegmentFind(A.KOD,0,2),'')
                                END,
                    LISTE     = CASE WHEN ISNULL(R3.KALIMBALMDEPLIST,0)=0          -- RRAB='K' Or 
                                     THEN ''
                                     ELSE ISNULL(dbo.Isd_SegmentFind(A.KOD,0,3),'')
                                END,
                    KMAG      = CASE WHEN ISNULL(R3.KALIMBALMDEPLIST,0)=0          -- RRAB='K' Or 
                                     THEN ''
                    ...
                    KalimLM   = CASE WHEN  ISNULL(R3.KALIMBALMDEPLIST,0)=0                                              THEN 0
                                     WHEN  ISNULL(R3.KALIMABLM67DEPLIST,0)=1 AND  CHARINDEX(LEFT(A.LLOGARIPK,1),'67')=0 THEN 0
                                     ELSE                                                                                    ISNULL(R3.KALIMBALMDEPLIST,0)
                                END
                 -- R1.KODAB
               FROM BANKASCR A LEFT JOIN LLOGARI B  ON A.LLOGARIPK = B.KOD
                               LEFT JOIN BANKA   R1 ON A.NRD=R1.NRRENDOR
                               LEFT JOIN BANKAT  R2 ON R2.KOD=R1.KODAB,CONFIGLM R3
              WHERE A.TIPKLL='T') A
