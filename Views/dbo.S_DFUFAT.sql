...
CREATE           VIEW [dbo].[S_DFUFAT] AS

  SELECT TOP 100 PERCENT 
         NRRENDOR     = CASE WHEN A.ORG='F' THEN A.NRRENDORDOK ELSE 0 END,
         A.KOD,
         A.PERSHKRIM,
         KMON         = ISNULL(A.KMON,''),
         ...
         AGJENTSHITJE = ISNULL(C.PRODUCTMANAGER,''),
...
/*       VLEFTAPROG   = (SELECT SUM(B.VLEFTA)   
                           FROM DFU B 
                          WHERE  CASE WHEN CHARINDEX ('.',B.KOD )>0 THEN CHARINDEX ('.',B.KOD)-1 ELSE LEN(B.KOD) END=CASE WHEN CHARINDEX ('.',A.KOD )>0 THEN CHARINDEX ('.',A.KOD)-1 ELSE LEN(A.KOD) END
                                 AND 
                                (ISNULL(B.DTFAT,0)<ISNULL(A.DTFAT,0) OR (ISNULL(B.DTFAT,0)=ISNULL(A.DTFAT,0) AND B.NRRENDOR<=A.NRRENDOR))                         
                                 AND 
                                (
                                (B.TREGDK='K' AND (B.VLEFTA>0 OR (B.VLEFTA=0 AND B.VLEFTAMV>0))) OR (B.TREGDK='D' AND (B.VLEFTA<0 OR (B.VLEFTA=0 AND B.VLEFTAMV<0))) 
                                 )), */
    ...
    FROM DFU A LEFT JOIN FURNITOR C ON LEFT(A.KOD, CASE WHEN CHARINDEX ('.',A.KOD )>0 THEN CHARINDEX ('.',A.KOD)-1 ELSE LEN(A.KOD) END)=C.KOD
   WHERE ( (A.TREGDK='K' AND (A.VLEFTA>0 OR (A.VLEFTA=0 AND A.VLEFTAMV>0))) OR (A.TREGDK='D' AND (A.VLEFTA<0 OR (A.VLEFTA=0 AND A.VLEFTAMV<0))) )
