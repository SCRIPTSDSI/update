...
CREATE     VIEW [dbo].[QFFFatVonPikezim] AS 
      ...
      SELECT NRRENDOR    = MAX(CASE WHEN A.ORG='S' THEN A.NRRENDORDOK ELSE 0 END),
             ...
             AGJENTSHITJE= MAX(B.PRODUCTMANAGER),
        ...
        FROM DFU A LEFT  JOIN FURNITOR B ON  B.KOD=LEFT(A.KOD, CASE WHEN CHARINDEX ('.',A.KOD )>0 THEN CHARINDEX ('.',A.KOD)-1 ELSE LEN(A.KOD) END)
                   LEFT  JOIN DFU      C ON (A.NRFAT  =C.NRFAT) AND (A.DTFAT  =C.DTFAT) AND (C.DATEDOK<=DBO.DATEVALUE('01/01/1955')) AND
                                            ...
                                            (A.NRRENDOR<>C.NRRENDOR) AND (A.TREGDK='K') AND (A.TIPDOK IN ('FF','SP'))
                   LEFT  JOIN FF      D ON A.NRRENDOR=D.NRDITAR
       WHERE A.TREGDK='K'
