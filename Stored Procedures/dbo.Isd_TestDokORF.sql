...
CREATE   procedure [dbo].[Isd_TestDokORF]
(
  @pDateKp    Varchar(20),
  @pDateKs    Varchar(20),
...
--      INTO #TestDok     
--     WHERE 1=2;
--      EXEC dbo.Isd_TestDokORF '01/01/2010','31/12/2012','ORF',0.01,'#TestDok';
...
-- Inicializimi
          IF OBJECT_ID('TempDB..#TestFtORF') IS NOT NULL
             DROP TABLE #TestFtORF;
          IF OBJECT_ID('TempDB..#TestORF') IS NOT NULL
             DROP TABLE #TestORF; 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/03',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Detaj',
			     ...
			     TableName  = @TableName,
                 A.NRRENDOR  
		    FROM ORF A Inner Join #TestORF A1 On A.NRRENDOR = A1.NRRENDOR
                       Inner Join ORFSCR B    On A.NRRENDOR = B.NRD 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/06',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Row',
                 ...
                 A.NRRENDOR 
		    FROM ORF A Inner Join #TestORF A1 On A.NRRENDOR = A1.NRRENDOR
                       Left  Join ORFSCR B    On A.NRRENDOR=B.NRD
        GROUP BY A.NRRENDOR
...
-------------------------- A f i s h i m i --------------------------
          ...
          IF @TestTable<>''      -- Mbush nje tabele qe mund te afishohet me vone ....
             ...
             BEGIN
               SELECT * FROM #TestFtORF ORDER BY ErrorOrder,ErrorRowNr,TableName;
             END;
GO
