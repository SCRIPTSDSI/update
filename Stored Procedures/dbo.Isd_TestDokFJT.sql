...
CREATE   procedure [dbo].[Isd_TestDokFJT]
(
  @pDateKp    Varchar(20),
  @pDateKs    Varchar(20),
...
--      INTO #TestDok     
--     WHERE 1=2;
--      EXEC dbo.Isd_TestDokFJT '01/01/2010','31/12/2012','FJT',0.01,'#TestDok';
...
-- Inicializimi
         IF OBJECT_ID('TempDB..#TestFtFJT') IS NOT NULL
            DROP TABLE #TestFtFJT; 
         IF OBJECT_ID('TempDB..#TestFJT')   IS NOT NULL
            DROP TABLE #TestFJT; 
     ...
     SELECT Dok        = REPLICATE('',20),  Test       = REPLICATE('',30),
            ...
            ErrorOrder = REPLICATE('',100), ErrorRowNr = CAST(0 AS BIGINT), NrRendor   = 0
       INTO #TestFtFJT    
      WHERE 1=2;
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/03',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Detaj',
			     ...
			     TableName  = @TableName,
                 A.NRRENDOR  
		    FROM FJT A Inner Join #TestFJT A1 On A.NRRENDOR = A1.NRRENDOR
                       Inner Join FJTSCR B    On A.NRRENDOR = B.NRD 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/06',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Row',
                 ...
                 A.NRRENDOR 
		    FROM FJT A Inner Join #TestFJT A1 On A.NRRENDOR = A1.NRRENDOR
                       Left  Join FJTSCR B    On A.NRRENDOR=B.NRD
        GROUP BY A.NRRENDOR
...
-------------------------- A f i s h i m i --------------------------
          ...
          ELSE
             BEGIN
               SELECT * FROM #TestFtFJT ORDER BY ErrorOrder,ErrorRowNr,TableName;
             END;
GO
