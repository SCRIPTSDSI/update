...
CREATE PROC [dbo].[_fiscalreportsalexmlposbak]  
	@Id						INT
...
SELECT       @VatRegistrationNo = (SELECT TOP 1 ISNULL(VLERA,'') FROM KONFIG WHERE FUSHA = 'FISCVATREGISTRATIONNO')
		   , @BusinessUnit = (SELECT TOP 1 fiscbusunitcode from kase A inner join smbak B on a.kmag=b.kmag where b.nrrendor=@id)
		--,@BusinessUnit      = (SELECT FISCBUSUNITCODE FROM POSKONFIGMAG A INNER JOIN SMBAK B ON A.KMAG=B.KMAG WHERE B.NRRENDOR=@ID)--(SELECT TOP 1 ISNULL(VLERA,'') FROM KONFIG WHERE FUSHA = 'FISCBUSINESSUNIT')
	...
	SELECT  @PERQZBR =  PERQZBR FROM SMBAK WHERE NRRENDOR = @ID
	--hard coded parameters
	SET @CashRegister = (SELECT TOP 1 U.FISCTCRNUM FROM kase U INNER JOIN SMBAK P ON P.kase = U.kod WHERE P.NRRENDOR = @Id);
	SET @OperatorCode = (SELECT TOP 1 U.OPERATORCODE FROM DRH..USERS U 
	INNER JOIN kase K ON K.KOD = U.DRN 
	INNER JOIN SMBAK P ON P.kase = K.kod 
	WHERE P.NRRENDOR = @Id);

	
	update SMBAK set fisctcr = @CashRegister, fiscbusinunit=@BusinessUnit where nrrendor = @id
	 ...
	 (
		  ...
		  UPDATE SMbak SET NRFISKALIZIM=@NRFISKALIZIM WHERE NRRENDOR=@ID and NRFISKALIZIM is null
	...
	INTO #FJSCR 
	FROM SMBAK F
	INNER JOIN SMBAKSCR S ON F.NRRENDOR = S.NRD
	INNER JOIN ARTIKUJ A ON A.KOD = S.KARTLLG
	INNER JOIN KLASATVSH KT ON KT.KOD = A.KODTVSH
	WHERE NRD = @Id;

	declare @tipnipt as varchar(50)
	select top 1 @tipnipt = tipnipt  from SMBAK where nrrendor= @Id

	declare @kurs2 as float
	select @kurs2 = kurs2 from SMBAK where nrrendor = @id

	--select KURS2 ,* from SMBAK
	...
	--eshte shtuar e re
	if(select top 1 isnull(iic,'') from SMBAK where nrrendor = @Id)=''
	begin
		...
		select @UUID = UUID, @iscash = isnull(iscash,1),@isfj= isnull(ISFJ,0) from SMBAK where nrrendor = @id
				...
				/*ALBAN*/	   ,@NRFISKALIZIM	AS '@InvOrdNum'						
							  ...
							  -- ,CASE WHEN @TIPFISKAL='VAT' THEN CONVERT(DECIMAL(20, 2), 0) else CONVERT(DECIMAL(20, 2), S.VLERTOT) end	AS '@TaxFreeAmt'		
							   ...
							   ,
								...
								CASE WHEN EXISTS(SELECT 1 FROM SMBAK WHERE ISNULL(RELATEDFIC,'')<>'' AND NRRENDOR = @Id ) THEN
								(
									...
									FROM SMBAK N WHERE N.RELATEDFIC = S.RELATEDFIC
								...
								(	--nga klienti
									SELECT	REPLACE(s.SHENIM1, '"', '')  AS 'Buyer/@Name',		-- OPTIONAL| MANDATORY B2B: 
											s.NIPT						 AS 'Buyer/@IDNum',		-- OPTIONAL| MANDATORY B2B: 
																								...
																								*/
											s.TIPNIPT	 AS 'Buyer/@IDType',	-- OPTIONAL| MANDATORY B2B: 
											...
											ISNULL(LEFT(S.COUNTRY, 3), ISNULL(C.ADRESA3, 'ALB')) AS 'Buyer/@Country'	-- OPTIONAL| MANDATORY IF NAME IS FILLED: 	
									FROM KLIENT C 
	...
	update smbak set RESPONSEXMLFIC = @responseXML
	where NRRENDOR = @ID
	set nocount on

	select top 1 @tipnipt = tipnipt from  smbak where nrrendor= @Id

	if (select count(1) from smbak where isfj = 1 and isnull(iscash,1)=0 and isnull(eic,'')='' and @tipnipt='NUIS' and nrrendor = @Id)=1
	begin
