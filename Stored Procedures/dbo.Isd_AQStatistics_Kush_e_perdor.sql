...
CREATE        procedure [dbo].[Isd_AQStatistics_Kush_e_perdor]
(
  @pKod       VARCHAR(60),
...
AS
   ...
   -- SELECT B.* FROM AQ A INNER JOIN AQSCR B ON A.NRRENDOR=B.NRD WHERE KARTLLG=@sKod;
      ...
      SELECT DATEFIRSTCE   = MIN(CASE WHEN ISNULL(B.KODOPER,'') IN ('CE')            THEN  B.DATEOPER END),
             ...
             DATEFIRSTSH   = MIN(CASE WHEN ISNULL(B.KODOPER,'') IN ('SH','JP','CR')  THEN  B.DATEOPER END),
             DATELASTSH    = MIN(CASE WHEN ISNULL(B.KODOPER,'') IN ('SH','JP','CR')  THEN  B.DATEOPER END),
             ...
             VLERA         = SUM(CASE WHEN B.KODOPER IN ('CE','BL','MM','SI','ST')   THEN  1
                                      WHEN B.KODOPER IN ('SH','JP','CR')             THEN -1
                                      ELSE                                                 0
                                 END * B.VLERABS),
