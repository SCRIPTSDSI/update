...
CREATE   procedure [dbo].[Isd_TestDokSM]
(
  @pDateKp    Varchar(20),
  @pDateKs    Varchar(20),
...
--      INTO #TestDok     
--     WHERE 1=2;
--      EXEC dbo.Isd_TestDokSM  '01/01/2010','31/12/2012','SM',0.01,'#TestDok';
...
-- Inicializimi
          IF OBJECT_ID('TempDB..#TestFtSM') IS NOT NULL
             DROP TABLE #TestFtSM; 
          IF OBJECT_ID('TempDB..#TestSM') IS NOT NULL
             DROP TABLE #TestSM; 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/03',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Detaj',
			     ...
			     TableName  = @TableName,
                 A.NRRENDOR  
		    FROM SM A Inner Join #TestSM A1 On A.NRRENDOR = A1.NRRENDOR
                      Inner Join SMSCR B    On A.NRRENDOR = B.NRD 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/06',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Row',
                 ...
                 A.NRRENDOR 
		    FROM SM A Inner Join #TestSM A1 On A.NRRENDOR = A1.NRRENDOR
                      Left  Join SMSCR B    On A.NRRENDOR=B.NRD
        GROUP BY A.NRRENDOR
...
-------------------------- A f i s h i m i --------------------------
          ...
          IF @TestTable<>''      -- Mbush nje tabele qe mund te afishohet me vone ....
             ...
             BEGIN
               SELECT * FROM #TestFtSM ORDER BY ErrorOrder,ErrorRowNr,TableName;
             END;
GO
