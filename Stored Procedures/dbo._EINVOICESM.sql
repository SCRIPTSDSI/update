...
BEGIN	
		...
		INNER JOIN SM S ON S.KASE = K.KOD
		...
		SELECT @DATE		= dbo.DATE_1601(GETDATE()),		--> kujdes data duhet edhe me pjesen e ORE-s
	...
	SELECT TOP 1 
		...
		*/
		(SELECT   
		           ...
		           , CONVERT(DECIMAL(20, 2), SUM(convert(decimal(20,2),SMSCR.VLPATVSH))) -  CONVERT(DECIMAL(20, 2), SUM(convert(decimal(20,2),SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100))))) AS 'cbc:TaxAmount'
		...
		(SELECT   
				...
				--  , CONVERT(DECIMAL(20, 2), (SELECT SUM(FJSCR.VLTVSH) FROM FJSCR WHERE NRD=S.NRRENDOR )) AS 'cbc:TaxAmount'
				 -- , 
				  ...
				  , CONVERT(DECIMAL(20, 2), SUM(convert(decimal(20,2),SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100))))) AS 'cac:TaxSubtotal/cbc:TaxableAmount'
				  , KMON AS 'cac:TaxSubtotal/cbc:TaxAmount/@currencyID'
				  , CONVERT(DECIMAL(20, 2), SUM(convert(decimal(20,2),SMSCR.VLPATVSH))) -  CONVERT(DECIMAL(20, 2), SUM(convert(decimal(20,2),SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100))))) AS 'cac:TaxSubtotal/cbc:TaxAmount'
				  , CASE WHEN PERQTVSH=20 THEN  'S' 
						 WHEN PERQTVSH=0  THEN  'Z'
				  ...
				  , CASE WHEN  PERQTVSH>=0 THEN NULL ELSE 'AAM' END AS 'cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode'
				  , CASE WHEN  PERQTVSH>=0 THEN NULL ELSE 'Arsyeja per tvsh 0' END AS 'cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReason'
				 , 'VAT' AS 'cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID'
			FROM SMSCR 
			WHERE NRD = S.NRRENDOR 
			GROUP BY SMSCR.PERQTVSH
			ORDER BY SMSCR.PERQTVSH DESC
			FOR XML PATH(''), TYPE
		)  AS 'cac:TaxTotal',
		...
		CONVERT(DECIMAL(20, 2), (SELECT SUM(CONVERT(DECIMAL(20, 2),SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100)))) FROM SMSCR WHERE SMSCR.NRD = S.NRRENDOR)) AS 'cac:LegalMonetaryTotal/cbc:LineExtensionAmount', -- Totali i të gjitha shumave neto për artikujt në një Faturë
		KMON AS 'cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount/@currencyID',		
		CONVERT(DECIMAL(20, 2), (SELECT SUM(CONVERT(DECIMAL(20, 2),SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100)))) FROM SMSCR WHERE SMSCR.NRD = S.NRRENDOR)) AS 'cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount',  -- Shuma totale e faturës pa TVSH
		...
		(SELECT   KARTLLG AS 'cac:InvoiceLine/cbc:ID',
				  ...
				  CONVERT(DECIMAL(20, 2), (SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100)))) AS 'cac:InvoiceLine/cbc:LineExtensionAmount',
				 ...
				 'VAT' AS 'cac:InvoiceLine/cac:Item/cac:ClassifiedTaxCategory/cac:TaxScheme/cbc:ID',
				  ...
				  CONVERT(DECIMAL(20, 2),  (SMSCR.VLPATVSH/(cONVERT(FLOAT,(PERQTVSH+100)/100)))/SASI) AS'cac:InvoiceLine/cac:Price/cbc:PriceAmount',		 
...
set @hDoc = 1
 ...
 IF (@Error = 0)
		 BEGIN
			...
			UPDATE sm SET eic = @EIC
						  , RESPONSEXMLEIC	= @responseXml
			WHERE NRRENDOR = @NrRendor;
		 END
else
begin
 ...
 --ALTER TABLE SM ADD RESPONSEXMLFIC XML NULL, RESPONSEXMLEIC XML NULL
GO
