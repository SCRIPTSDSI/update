...
CREATE        procedure [dbo].[Ehw_F5_Palm_Exp0]
  (
...
Exec('INSERT INTO '+@VServer+'ListeArtikuj (KOD,EMERTIM,EMERTIMSH, 
      ...
      SELECT KOD, 
        ...
        FROM '+@VTbNameO+' 
       WHERE (LEFT(KOD,1) IN (''A'',''P'',''S''))')
--WHERE (LEFT(KOD,1) IN (''A'',''P'',''S'')) AND (KOD >='+@PKodArtKp+') AND (KOD<='+@PKodArtKs+')' 

--Set @VString         = QuoteName('A','''')+','+QuoteName('P','''')+','+QuoteName('S','''')
--Exec('INSERT INTO '+@VServer+'ListeArtikuj (KOD,EMERTIM,EMERTIMSH, 
--                                            CMSH1,CMSH2,CMSH3,CMSH4,CMSH5,CMSH6,CMSH7,CMSH8,CMSH9,CMSH10,CMSHPLM1,CMSHPLM2,
--                                            KLASIF1,KLASIF2,KLASIF3,NJESI,BARKOD,NOTACTIV)
--      SELECT KOD, 
--             Replace(LEFT(LTRIM(RTRIM(PERSHKRIM)),90),'''+@VThonjez+''','''+@VBosh+'''),
--             Replace(PERSHKRIMSH,'''+@VThonjez+''','''+@VBosh+'''),
--             CMSH, CMSH1,CMSH2,CMSH3,CMSH4,CMSH5,CMSH6,CMSH7,CMSH8,CMSH9,CMSHPLM1,CMSHPLM2,
--             KLASIF,KLASIF2,KLASIF3,NJESI,BC,NOTACTIV
--        FROM '+@VTbNameO+' 
--       WHERE LEFT(KOD,1) IN ('+@VString+')')
--
--Replace(LEFT(PERSHKRIM,90),'','','' '')
...
Exec('INSERT INTO '+@VServer+'ListeKlient (KOD,PERSHKRIM, PERFAQESUES, ADRESA1,ADRESA2,ADRESA3, 
      ...
      SELECT A.KOD,A.PERSHKRIM,A.PERFAQESUES, A.ADRESA1,B.PERSHKRIM,A.ADRESA3,
...
Exec('INSERT INTO '+@VServer+'MGExpF5   
      ...
      SELECT KMAG,KMAG,NRDOK,DATEDOK,KARTLLG,SASI,C.CMSH,0,''Furnizim Ditor'',''FU'',B.NRRENDOR
       ...
       WHERE (KMAG >= '+@PKodMagKp+') AND (KMAG<='+@PKodMagKs+') AND 
             ...
             (DST=''FU'')
    ORDER BY KMAG,NRDOK,DATEDOK,B.KOD')
--Shtim i Artikujve qe mungojne..
Exec('INSERT INTO '+@VServer+'MGExpF5   
            (KODKL,KMAG,NRDOK,DATEDOK,KOD,SASI,CMIM,DHURATE,SHENIM1,LLOJDOK,NRRENDORF5) 
      SELECT B.KOD,B.KOD,0,NULL,A.KOD,0,A.CMSH1,0,''Furnizim Ditor'',''FU'',0 
        FROM '+@VServer+'ListeArtikuj A,'+@VServer+'ListeMagazina B
       WHERE NOT EXISTS(SELECT NRRENDOR FROM '+@VServer+'MGExpF5 C WHERE (A.KOD=C.KOD) AND (B.KOD=C.KMAG))')
...
Set @VTbNameO        = @VDbFin+@VTbName
Exec('INSERT INTO '+@VServer+'MGGjendjeF5 (KMAG,KOD,SHENIM1,SASI,NJESI,CMIM)
      SELECT A.KMAG,A.KARTLLG,MAX(A.PERSHKRIM), ROUND(SUM(A.SASIH-SASID),3),MAX(A.NJESI),ROUND(MAX(B.CMSH),3)
