...
CREATE PROC [dbo].[__eInvoiceGetRequestStatusGroup]
 	@Eic			  NVARCHAR(100)	=	 '',
...
BEGIN TRY
	...
	IF OBJECT_ID('TEMPDB..#FJFISCAL1') IS NOT NULL
			   DROP TABLE #FJFISCAL1;
	...
	BEGIN
		...
		BEGIN
			...
			EXEC ('
		     UPDATE A 
			    SET A.FISSTATUS = C.[STATUS]
			   FROM FJ A INNER JOIN '+@PTablename+' B ON A.NRRENDOR=B.NRRENDOR 
			             INNER JOIN #FJFISCAL1      C ON A.FISEIC=C.EIC	');  -- @PTablename vjen nga programi i gatshem per ato te selectuara
