...
CREATE PROCEDURE [dbo].[FJFROMSMANULLIM](@NRRENDORSM AS INT
                        )
                        AS
                        DECLARE @KODKLI as varchar(10),@DATFAT as DATETIME,
                        @DATESHOQ as DATETIME,@NRSERIAL as VARCHAR(50),@SHENIM AS VARCHAR(250)='',@PARAPG AS FLOAT = 0,
						@ISFJ AS BIT
						SET @ISFJ=(SELECT ISFJ FROM SM WHERE NRRENDOR =@NRRENDORSM)
						IF @ISFJ=1
						BEGIN

						SELECT @KODKLI= KODFKL,@DATFAT = DATEDOK,@DATESHOQ=DATEDOK,@NRSERIAL = '',@SHENIM = SHENIM1 FROM SM WHERE NRRENDOR=@NRRENDORSM
			...
			--exec fj_insert_fd @PNRRENDOR
                      ...
                      --  @PKODUSER,@PDATEDOK,@PKODFKL,@PKMON,@PKMAG,@PKURS2,@NRSERIAL,@NRFAT--,@VLERTOT,@PKASE,@VLERZBR
						...
						declare @tipnipt varchar(100),@iscash as bit

						select @tipnipt = TIPNIPT,@iscash = isnull(iscash,0) from sm where nrrendor = @NRRENDORSM

						if @tipnipt = 'NUIS' and @iscash=0
						begin
							exec dbo._EINVOICESM @NRRENDORSM
						end
