...
CREATE Procedure [dbo].[Isd_SistemimMg2]    
(
...
-- Njesoj si Isd_SistemimMg por referohet jo ARTIKUJSISTM,ARTIKUJSIST por tabeles SISTEMIMMG dhe SISTEMIMMGSCR
     ...
     DECLARE @NewID        Int, 
      ...
      SELECT 'H',A.KMAG,B.NRRENDOR,A.NRDOK,A.NRFRAKS,A.DATEDOK,A.SHENIM1,
             'Periudhe deri '+REPLACE(CONVERT(Varchar(30),A.DATEDOK,103),'/','.'),
             '','',@QKosto,B.GRUP,
             'SI','',0,0,'','',0,0,0,'','',0 
        FROM SISTEMIMMG A LEFT JOIN MAGAZINA B ON A.KMAG=B.KOD
