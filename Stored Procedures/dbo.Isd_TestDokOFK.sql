...
CREATE   procedure [dbo].[Isd_TestDokOFK]
(
  @pDateKp    Varchar(20),
  @pDateKs    Varchar(20),
...
--      INTO #TestDok     
--     WHERE 1=2;
--      EXEC dbo.Isd_TestDokOFK '01/01/2010','31/12/2012','OFK',0.01,'#TestDok';
...
-- Inicializimi
          IF OBJECT_ID('TempDB..#TestFtOFK') IS NOT NULL
             DROP TABLE #TestFtOFK; 
          IF OBJECT_ID('TempDB..#TestOFK')   IS NOT NULL
             DROP TABLE #TestOFK; 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/03',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Detaj',
			     ...
			     TableName  = @TableName,
                 A.NRRENDOR  
		    FROM OFK A Inner Join #TestOFK A1 On A.NRRENDOR = A1.NRRENDOR
                       Inner Join OFKSCR B    On A.NRRENDOR = B.NRD 
     ...
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/06',
       ...
       FROM
	    ( SELECT Test       = @TableName+'->Row',
                 ...
                 A.NRRENDOR 
		    FROM OFK A Inner Join #TestOFK A1 On A.NRRENDOR = A1.NRRENDOR
                       Left  Join OFKSCR B    On A.NRRENDOR=B.NRD
        GROUP BY A.NRRENDOR
...
-------------------------- A f i s h i m i --------------------------
          ...
          IF @TestTable<>''      -- Mbush nje tabele qe mund te afishohet me vone ....
             ...
             BEGIN
               SELECT * FROM #TestFtOFK ORDER BY ErrorOrder,ErrorRowNr,TableName;
             END;
GO
