...
CREATE PROC [dbo].[__FiscalCreateSalesXmlFJ2_COPY]
	 @NrRendor		INT,
...
begin try
			...
			--PER TU NDERRUAR NE RAST TJETER
			   ...
			   DECLARE     @VatRegistrationNo	VARCHAR(50)
				...
				SELECT   @VatRegistrationNo	= (SELECT TOP 1 ISNULL(VLERA,'') FROM FisConfig WHERE FUSHA = 'FISCVATREGISTRATIONNO')
				...
				--SELECT @VatRegistrationNo, @BusinessUnit, @SoftNum, @ManufacNum, @schema , @FiscUrL , @Certificate ;
				...
				--alter table fj add FISCQRCODELINK varchar(max) null
				...
				--PRINT 'B'+@BusinessUnit
	
				--PRINT 'C'+@CashRegister
				...
				(
						SELECT 
							   ...
							   ,CONVERT(DECIMAL(18, 2), VLPATVSH*S.KURS2)	AS '@TotPriceWoVAT'	-- MANDATORY: Total price of the invoice excluding VAT.
							   --,CONVERT(DECIMAL(18, 2), VLERTOT)	AS '@TotPriceWoVAT'
							   ,CONVERT(DECIMAL(18, 2), VLTVSH*S.KURS2)		AS '@TotVATAmt'		-- MANDATORY: Total VAT amount of the invoice. 
							   --,CONVERT(DECIMAL(18, 2), 0) AS '@TotVATAmt'
							   ,CONVERT(DECIMAL(18, 2), VLERTOT*S.KURS2)	AS '@TotPrice'		-- MANDATORY: Total price of all items including taxes and discounts.
							   ...
							   ,
								...
								(
									SELECT  ISNULL(FREF.FISIIC,'') AS '@IICRef',				-- IIC reference on the original invoice.
							 				DBO.DATE_1601(FREF.DATECREATE) AS '@IssueDateTime',		-- Date and time the original invoice is created and issued at TCR.
											CASE WHEN ISNULL(LLOJDOK,'')='' THEN 'CORRECTIVE' ELSE 'DEBIT' END AS '@Type'				-- Type of the corrective invoice.
									FROM FJ FREF WHERE ISNULL(FREF.FISIIC,'') = ISNULL(S.FISRELATEDFIC,'')  
									AND NRRENDOR=@NrRendor AND ISNULL(S.FISRELATEDFIC,'')<>''
							   ...
							   ,														-- OPTIONAL:  
								...
								(														-- MANDATORY: 
									...
									SELECT CONVERT(DECIMAL(18, 2), S.VLERTOT*S.KURS2) AS 'PayMethod/@Amt',
							   ...
							   ,(	--nga config -- SELECT * FROM CONFND
									SELECT PERSHKRIM AS 'Seller/@Name',					-- MANDATORY: 
										   ...
										   ISNULL(SHENIM2,'') AS 'Seller/@Town',					 -- MANDATORY FOR FOREIGNER:    QYTETI
								...
								,
									(	SELECT  KARTLLG AS 'I/@C',								-- OPTIONAL:  Code of the item from the barcode or similar representation
											...
											CONVERT(DECIMAL(18, 2), VLERABS*S.KURS2) AS 'I/@PA',		-- MANDATORY: Total price of goods after the tax and applying discounts Negative values allowed when CorrectiveInv or BadDebtInv exist.
											CONVERT(DECIMAL(18, 2), VLPATVSH*S.KURS2) AS 'I/@PB',		-- MANDATORY: Total price of goods and services before the tax Negative values allowed when CorrectiveInv or BadDebtInv exist.
											...
											CONVERT(DECIMAL(18, 2), CMIMBS*S.KURS2) AS 'I/@UPB',		-- MANDATORY: Unit price before Value added tax is applied
											--CONVERT(DECIMAL(18, 2), CMIMBS) AS 'I/@UPA',		-- MANDATORY: Unit price after Value added tax is applied
											CONVERT(DECIMAL(18, 2), VLERABS/SASI*S.KURS2) AS 'I/@UPA',
											...
											CONVERT(DECIMAL(18, 2), VLTVSH*S.KURS2)  AS 'I/@VA',		-- MANDATORY: Amount of value added tax for goods and services. Must not exists if IsIssuerInVAT equals false and is not reverse charge or self-invoice. Mandatory if IsReverseCharge equals true. Negative values allowed when CorrectiveInv or BadDebtInv exist.
								...
								(	SELECT  CONVERT(VARCHAR(10), CONVERT(DECIMAL(18, 0), COUNT(1)))	  AS 'SameTax/@NumOfItems',
											CONVERT(DECIMAL(18, 2), SUM(VLPATVSH*@KURSFAT))					  AS 'SameTax/@PriceBefVAT',
											CONVERT(DECIMAL(18, 2), PERQTVSH)					   	  AS 'SameTax/@VATRate',
											--APLTVSH													  AS 'SameTax/@ExemptFromVAT',		-- nuk duhet APLTVSH
																													-- Exempt from VAT.
																														-- TYPE_1 Exempt type 1. Exempted on the basis of Article 51 of the VAT law 
																														-- TYPE_2 Exempt type 2. Exempted on the basis of Articles 53 and 54 of the VAT law 
											CONVERT(DECIMAL(18, 2), SUM(VLTVSH*@KURSFAT))					  AS 'SameTax/@VATAmt'
									FROM #FJSCR
