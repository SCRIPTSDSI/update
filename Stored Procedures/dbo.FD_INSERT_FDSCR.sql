...
CREATE    PROCEDURE [dbo].[FD_INSERT_FDSCR]
(
@PNrRendor INT,        --Nrrendori i FD-se 
...
SELECT 
       NRD=@PNrRendor,
       KOD       =(SELECT KMAG FROM FD A WHERE A.NRRENDOR=@PNrRendor)+'.'+@KODART+'...',
       KODAF     =@KODART,
       KARTLLG   =@KODART,
       ...
       KOMENT    =(SELECT SHENIM1   FROM FD A WHERE A.NRRENDOR=@PNrRendor),
       PROMOC    =0,
       PROMOCTIP ='',
       RIMBURSIM =0,
       DTSKADENCE=NULL,
...
IF ISNULL((SELECT TOP 1 NRRENDOR FROM LMG A WHERE A.KOD=(SELECT KOD FROM FDSCR B WHERE B.NRRENDOR=@VNrRendorScr)),0)=0
  INSERT INTO LMG (KOD,PERSHKRIM,KMON,NRMAG,KMALL,SASI,VLERE,SASIM,VLEREM,SASITMP,VLERETMP,
       ...
       SELECT      KOD=A.KOD,PERSHKRIM=A.PERSHKRIM,KMON='',NRMAG=0,KMALL='',SASI=0,VLERE=0,SASIM=0,VLEREM=0,SASITMP=0,VLERETMP=0,
                   SG1=(SELECT KMAG FROM FD B WHERE B.NRRENDOR=@PNrRendor),SG2=@KODART ,SG3='',SG4='',SG5='',SG6='',SG7='',SG8='',SG9='',SG10='',TROW=0,TAGNR=0
       FROM FDSCR A
