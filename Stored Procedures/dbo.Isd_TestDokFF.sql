...
CREATE   procedure [dbo].[Isd_TestDokFF]
(
  @pDateKp         Varchar(20),
  @pDateKs         Varchar(20),
...
--      INTO #TestDok     
--     WHERE 1=2;
--      EXEC dbo.Isd_TestDokFF '01/01/2010','31/12/2012','FF',0.01,'#TestDok';
...
-- Test 9.     Test per Kod, KMon ne Ditar, Monvendi dhe kurse ne Ditar (Njesoj si Testi 8)
     ...
     DECLARE @DateKp      DateTime,
         ...
         IF  dbo.Isd_ListFields2Lists(@PTNames,'ALL,BL,'+@TableName,'')=''
             RETURN;

-- Inicializimi
          IF OBJECT_ID('TempDB..#TestFtFF') IS NOT NULL
             DROP TABLE #TestFtFF; 
          IF OBJECT_ID('TempDB..#TestFF')   IS NOT NULL
             DROP TABLE #TestFF; 
          IF OBJECT_ID('TempDB..#TestDtFF') IS NOT NULL
             DROP TABLE #TestDtFF; 
	   ...
	   (SELECT Test      = @TableName+'->FH',
		  ...
		  FROM 
			  ( SELECT                              NrDokFt   = MAX(A.NRDOK),  DateDokFt   = MAX(A.DATEDOK), 
				  ...
				  FROM FF A INNER JOIN #TestFF A1 ON A.NRRENDOR=A1.NRRENDOR
                            LEFT  JOIN FFSCR C    ON A.NRRENDOR=C.NRD AND C.TIPKLL='K'
				 WHERE ISNULL(A.KMAG,'')<>'' 
			  ...
			  ( SELECT KMagMg       = MAX(A.KMAG),    NrDokMg   = MAX(A.NRDOK),  DateDokMg = MAX(A.DATEDOK),FrDokMg   = MAX(ISNULL(A.NRFRAKS,0)),
				  ...
				  FROM FH A LEFT JOIN FHSCR C ON A.NRRENDOR=C.NRD AND ISNULL(C.GJENROWAUT,0)=0
			     WHERE DateDok>=@DateKp AND DateDok<=@DateKs
...
-- T2.                           Elementet e Fatures me ato te dokumentit magazine 
  ...
  UNION ALL
     SELECT @Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,@ErrorOrd+'/02',
       ...
       FROM
	  ( SELECT Test       = @TableName+'->FH Detaj',
			   ErrorDok   = @TableName+' '+CONVERT(nVarchar(20),CAST(A1.NrDokFt AS BIGINT))+', '+CONVERT(Varchar,A1.DateDokFt,4),
			   ErrorMsg   = CASE --WHEN B1.NrRendorMg IS NULL  THEN 'Mungon dok mag.'
...
-- T4.                           Referenca te panjohura Klient,Furnitor,Monedhe,Artikuj,Magazine,Ardhura,Shpenzime,Sherbime,Aktive etj.
     INSERT INTO #TestFtFF  
		   (Dok,       Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,ErrorOrder,ErrorRowNr)
  ...
  UNION ALL
	...
	( 	  SELECT Test       = @TableName+'->Ref',
			     ...
			     ErrorRef   = CASE WHEN (NOT EXISTS (SELECT KOD FROM FURNITOR B WHERE B.KOD=A.KODFKL)) THEN 'Furn: '+ISNULL(A.KODFKL,'')
...
-- T8.                           Test per Kod, KMon ne dokument, Monvendi dhe kurse ne dokument
     INSERT INTO #TestFtFF  
		   (Dok,       Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,ErrorOrder,ErrorRowNr)
...
-- T9.                           Test per Kod, KMon ne Ditar, Monvendi dhe kurse ne Ditar (Njesoj si Testi 8)
  ...
  UNION ALL
	...
	( 	  SELECT Test       = @DitarName+' - Ditar',
			     ...
			     TableName  = @TableName,
                 A.NRRENDOR 
		    FROM DFU A INNER JOIN #TestDtFF A1 ON A.NRRENDOR = A1.NRRENDOR
...
-------------------------- A f i s h i m i --------------------------
          ...
          IF @PTestTable<>''       -- Mbush nje tabele qe mund te afishohet me vone ....
             BEGIN
               SET   @sSql = '
              INSERT INTO '+@PTestTable+'
                    (Dok,Test,ErrorDok,ErrorMsg,ErrorRef,TableName,NrRendor,ErrorOrder,ErrorRowNr)
          ...
          ELSE
             BEGIN
               SELECT * FROM #TestFtFF ORDER BY ErrorOrder,ErrorRowNr,TableName;
             END;
GO
