...
-- Exec [Isd_UpdateFhFromFtAmb] 76156,'',''


CREATE         Procedure [dbo].[Isd_UpdateFhFromFtAmb]
   ...
   --   From CONFIGLM;
      ...
      Select @KMagAmb = KMAGAMB
        From CONFIGMG;
         ...
         Set @KMagNew = IsNull(@KMagAmb,'');


      Select @GrupMgFt = Case When CharIndex(Left(LTrim(RTrim(IsNull(GRUP,'A'))),1),'ABCDEFGHIJ')>0 
...
-- End --
     ...
     -- ska nevoje sepse pyetet tek dbo.Isd_GjenerimFhFromFtAmb
     --  if @ChangeScr=1 or @ChangeDoc=0      -- Rasti @ChangeScr=1 Trajtohet tek Isd_GjenerimFhFromFtAmb
     --     Return;
-- End --
...
--             Print 'U Update Fh nga procedure Update '
      ...
      Update A
         Set A.NRMAG        = @NrMagAmb,          
             A.KMAG         = @KMagNew,
             A.DATEDOK      = B.DTDMAG,
             A.NRDOK        = B.NRDMAG,           -- Llogaritet
             A.NRFRAKS      = B.FRDMAG,           -- Llogaritet
             A.SHENIM1      = B.SHENIM1,
             A.SHENIM2      = B.SHENIM2,
             A.SHENIM3      = B.SHENIM3,
             A.SHENIM4      = B.SHENIM4,
