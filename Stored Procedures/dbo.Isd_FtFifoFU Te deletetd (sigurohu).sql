...
CREATE       Procedure [dbo].[Isd_FtFifoFU Te deletetd (sigurohu)] 
...
AS
      ...
      if Exists (Select [Name]
                  ...
                  Where Object_Id = Object_Id('TempDB..#FtID')) 
         Drop Table #FtID

  SELECT NRRENDOR
    INTO #FtID
    FROM DFU
   WHERE TREGDK='K' And 1=1
  ...
  SELECT A.KMON,
         ...
         DITENGAAFAT  = DATEDIFF(DAY,DATEADD(DAY,CASE WHEN A.TIPDOK='FF' THEN ISNULL(A1.DTAF,0)
                                     ...
                                     GETDATE()),
         A1.NRSERIAL
    FROM DFU A INNER JOIN #FtID A0 ON  A.NRRENDOR=A0.NRRENDOR
               LEFT  JOIN FF    A1 ON (A.NRRENDOR=A1.NRDITAR AND A.TIPDOK ='FF')
