...
-- Exec [Dbo].[Isd_KodChangeList] 'A001','A003',1,'','',1

CREATE Procedure [dbo].[Isd_KodChangeList]
(@PKod          Varchar(50),
 @PKodNew       Varchar(50),
 @PChangeName   Int,
...
-- CaseUpdate 1:   Nje Liste te vetme nga ekrani Liste
   ...
   if @PCaseUpdate<>1
      ...
      begin
          ...
          SELECT KOD,KODNEW,1,         TROW,TAGNR
            FROM KODCHANGE A
           WHERE TIPKLL='LST' AND 
                (ISNULL(KOD,'')<>'' AND ISNULL(KODNEW,'')<>'') AND
                (KOD<>KODNEW) --AND (NOT (EXISTS (SELECT KOD FROM LISTEB WHERE B.KOD=A.KODNEW)))
        ORDER BY KOD
      ...
      begin   -- Rasti nje Kod
        ...
        if IsNull((SELECT TOP 1 1 FROM #KODLIST),0)=0
           Return 


    UPDATE A
       SET A.PERSHKRIM = B.PERSHKRIM
      FROM #KODLIST A INNER JOIN LISTE B ON A.KODNEW=B.KOD
...
-- Grupi Faturime
       ...
       Set @SqlFilter00 = '
			UPDATE B
			   SET B.KODAF      = Dbo.Isd_SegmentNewInsert(B.KODAF,C.KODNEW,3),
			  ...
			  FROM FJ A INNER JOIN FJSCR             B ON A.NRRENDOR = B.NRD
						INNER JOIN '+@TableNameTmp+' C ON Dbo.Isd_SegmentFind(B.KODAF,0,3) = C.KOD
                        INNER JOIN LISTE             D ON C.KODNEW=D.KOD
...
-- Magazina FH,FD
 ...
 RaisError (@TName, 0, 1) with NoWait
	   ...
	   Set @SqlFilter00 = '
			UPDATE A
			   SET A.KODLM = Dbo.Isd_SegmentNewInsert(A.KODLM,C.KODNEW,3)
			  FROM FH A INNER JOIN '+@TableNameTmp+' C ON Dbo.Isd_SegmentFind(A.KODLM,0,3) = C.KOD
						INNER JOIN LISTE             D ON C.KODNEW=D.KOD
			 WHERE 1=1 '
       Set @TblList = 'FH,FD'
       Set @Ind     = 1
...
-- FK
       ...
       Set @SqlFilter00 = '
			UPDATE B
			   SET --B.LLOGARI    = Dbo.Isd_SegmentNewInsert(B.LLOGARI,C.KODNEW,3),
				   B.KOD        = Dbo.Isd_SegmentFind(B.KOD,0,1)+''.''+
                                  Dbo.Isd_SegmentFind(B.KOD,0,2)+''.''+
                                  C.KODNEW                          +''.''+
                                  Dbo.Isd_SegmentFind(B.KOD,0,4)+''.''+
                                  IsNull(B.KMON,'''')
                -- B.PERSHKRIM  = CASE WHEN C.CHANGENAME=1 THEN D.PERSHKRIM ELSE B.PERSHKRIM END
			  FROM FK A   INNER JOIN FKSCR             B ON A.NRRENDOR = B.NRD
					 	  INNER JOIN '+@TableNameTmp+' C ON Dbo.Isd_SegmentFind(B.KOD,0,3) = C.KOD
                          INNER JOIN LISTE             D ON C.KODNEW=D.KOD
             WHERE 1=1 '
       Set @TblList = 'FK,FKST'
       Set @Ind     = 1
...
-- R E F E R E N C A
       ...
       Set @SqlFilter00 = '
			UPDATE A
			   SET A.LISTE = C.KODNEW
			  FROM KLIENT A INNER JOIN '+@TableNameTmp+' C ON A.LISTE = C.KOD
             WHERE 1=1 '
...
-- LMG
       Set @SqlFilter01 = '
            UPDATE A 
               SET SG3 = C.KODNEW,
                   KOD = ISNULL(SG1,'''')+''.''+ISNULL(SG2,'''')+''.''+ISNULL(SG4,'''')+''.''+C.KODNEW+''.''+ISNULL(SG5,'''')
