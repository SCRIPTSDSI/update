SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO







CREATE   PROCEDURE [dbo].[SM_INSERT_SMSCR]
(
@PNrRendor INT,        --nrrendor i SM-se
@PTipKLL   VARCHAR(1), --'K' fikse sepse te gjitha shitjet jane me magazine
@KODART    VARCHAR(30),--kod artikulli
@CMIM      FLOAT,      --cmim pa tvsh
@SASI      FLOAT,      --sasia
@VPATVSH   FLOAT,      --vlera pa tvsh
@VTVSH     FLOAT,      --vlera tvsh
@VTOT      FLOAT       --vlera totale
)
AS


INSERT INTO SMSCR
      (NRD, 
       KOD,
       KODAF,
       KARTLLG,
       PERSHKRIM,
       NRRENDKLLG,
       LLOGARIPK,
       NJESI,
       CMSHZB0,
       CMIMM,
       SASI,
       PERQDSCN,
       CMIMBS,
       VLERABS,
       VLERAM,
       VLPATVSH,
       VLTVSH,
       PERQTVSH,
       KOEFSHB,
       NJESINV,
       TIPKLL,
       BC,
       KOMENT,
       NOTMAG,
       RIMBURSIM,
       DTSKADENCE,
       SERI,
       KODKR,
       TROW,
       TAGNR,
       VLTAX)

SELECT NRD=@PNrRendor,
       KOD=CASE @PTipKLL WHEN 'K' THEN (SELECT KMAG FROM SM A WHERE A.NRRENDOR=@PNrRendor)+'.'+@KODART+'...'+(SELECT KMON FROM SM A WHERE A.NRRENDOR=@PNrRendor) 
                         WHEN 'L' THEN @KODART+'...'+(SELECT KMON FROM SM A WHERE A.NRRENDOR=@PNrRendor)  
                         WHEN 'R' THEN @KODART END,
       KODAF=@KODART,
       KARTLLG=@KODART,
       PERSHKRIM= CASE @PTipKLL WHEN 'K' THEN (SELECT PERSHKRIM FROM ARTIKUJ A WHERE A.KOD=@KODART) 
                                WHEN 'L' THEN (SELECT PERSHKRIM FROM LLOGARI A WHERE A.KOD=@KODART)  
                                WHEN 'R' THEN (SELECT PERSHKRIM FROM SHERBIM A WHERE A.KOD=@KODART) END,
       NRRENDKLLG=CASE @PTipKLL WHEN 'K' THEN (SELECT NRRENDOR  FROM ARTIKUJ A WHERE A.KOD=@KODART) 
                                WHEN 'L' THEN (SELECT NRRENDOR  FROM LLOGARI A WHERE A.KOD=@KODART)  
                                WHEN 'R' THEN (SELECT NRRENDOR  FROM SHERBIM A WHERE A.KOD=@KODART) END,
       LLOGARIPK= CASE @PTipKLL WHEN 'K' THEN '' 
                                WHEN 'L' THEN @KODART  
                                WHEN 'R' THEN @KODART END,
       NJESI=     CASE @PTipKLL WHEN 'K' THEN (SELECT NJESI     FROM ARTIKUJ A WHERE A.KOD=@KODART) 
                                WHEN 'L' THEN ('')  
                                WHEN 'R' THEN (SELECT NJESI     FROM SHERBIM A WHERE A.KOD=@KODART) END,
       CMSHZB0 =@CMIM,
       CMIMM   =@CMIM,
       SASI    =@SASI,
       PERQDSCN=0,
       CMIMBS  =@CMIM,
       VLERABS =@VTOT,
       VLERAM  =@VPATVSH,
       VLPATVSH=@VPATVSH,
       VLTVSH  =@VTVSH,
       PERQTVSH=(SELECT PERQTATS FROM CONFIGLM),
       KOEFSHB=1,
       NJESINV=    CASE @PTipKLL WHEN 'K' THEN (SELECT NJESI     FROM ARTIKUJ A WHERE A.KOD=@KODART) 
                                 WHEN 'L' THEN ('')  
                                 WHEN 'R' THEN (SELECT NJESI     FROM SHERBIM A WHERE A.KOD=@KODART) END,
       TIPKLL=@PTipKLL,
       BC= CASE  @PTipKLL WHEN 'K' THEN ISNULL((SELECT BC FROM ARTIKUJ A WHERE A.KOD=@KODART),'') ELSE '' END,
       KOMENT=(SELECT SHENIM2 FROM SM A WHERE A.NRRENDOR=@PNrRendor),
       NOTMAG=0,
       RIMBURSIM=0,
       DTSKADENCE=NULL,
       SERI='',
       KODKR='',
       TROW=0,
       TAGNR=0,
       VLTAX=0


UPDATE SM SET
       VLPATVSH=(SELECT SUM(VLPATVSH) FROM SMSCR WHERE NRD=@PNrRendor),
       VLTVSH  =(SELECT SUM(VLTVSH)   FROM SMSCR WHERE NRD=@PNrRendor),
       VLERTOT =(SELECT SUM(VLERABS)  FROM SMSCR WHERE NRD=@PNrRendor)
  WHERE NRRENDOR=@PNrRendor



GO
