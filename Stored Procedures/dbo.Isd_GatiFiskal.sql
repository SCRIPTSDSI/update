...
CREATE PROCEDURE [dbo].[Isd_GatiFiskal]
(
 @pNrRendor Integer
)
AS
...
SET NOCOUNT ON;
	...
	SET @TIPKLIENT=(SELECT top 1 TIPNIPT  FROM FJ A INNER JOIN KLIENT B ON A.KODFKL=B.KOD WHERE A.NRRENDOR=@pNrRendor)
	...
	SET @SELF=(SELECT CASE WHEN ISNULL(A.KLASETVSH,'') IN ('SANG','SELF','SEXP') THEN  A.KLASETVSH
							ELSE  NULL END FROM FJ A WHERE A.NRRENDOR=@pNrRendor)
	...
	BEGIN	
		IF EXISTS(SELECT 1 FROM FJ WHERE NRRENDOR = @pNrRendor  AND FISKALIZUAR=1 AND ISNULL(FISEIC,'')<>'')
			BEGIN
				SET @OUTPUT1='0'
				SET @OUTPUT2='0'
			END
			...
			BEGIN
				...
				EXEC Isd_Fiscal @pNrRendor,1,@OUTPUT2 OUTPUT
				IF @OUTPUT2 <> '0'
	...
	ELSE
		BEGIN
				IF EXISTS(SELECT 1 FROM FJ WHERE NRRENDOR = @pNrRendor 
					...
					ELSE
						BEGIN
									EXEC Isd_Fiscal @pNrRendor,0,@OUTPUT1 OUTPUT
